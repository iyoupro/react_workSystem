...
const initTreeNode = (node, setExpandRecently, setExpand, expandNotice, isHead) => {
  if (isHead) {
    if (expandNotice) {
      setTimeout(() => {
  ...
  const expendButton = node.querySelector('.treeView-expand');
  ...
  expendButton.addEventListener('click', clickExpandButton);
};

const TreeNode = ({ data, parentData, head, depth, defaultExpand, childrenHead, childrenTail, isShow, ...props }) => {
  const node = useRef(null);
  const [isExpand, setExpand] = useState(depth < defaultExpand);
  ...
  ReactUtil.init(() => initTreeNode(node.current, setExpandRecently, setExpand, expandNotice, depth === 0));
  ...
  return (
    ...
    >
      <Node data={data} parentData={parentData} {...props} direction={direction} treeNodeSize={treeNodeSize} isExpand={isExpand} isExpandRecently={isExpandRecently}/>
      ...
      {lineOption && isExpand && children && children.length > 1 && // 所有子节点之间的长线
        <div
          className={styles.line}
          style={{
      ...
      >
        {children && children.length > 0 && children.map((childNode, index) => 
          <TreeNode
            key={childNode.key}
            data={childNode}
            parentData={data}
